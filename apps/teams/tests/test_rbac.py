...
from apps.teams.roles import ROLE_ADMIN, ROLE_MANAGER, ROLE_OPERATOR, ROLE_OWNER, ROLE_VIEWER, has_permission, is_admin
...
class RBACLogicTest(TestCase):
    ...
    def test_has_permission(self):
        ...
        for user in [self.owner, self.admin, self.manager, self.operator, self.viewer]:
            self.assertTrue(
                has_permission(user, self.team, "view_dashboard"), f"{user.email} should have view_dashboard"
            )
        ...
        for user in [self.owner, self.admin, self.manager]:
            self.assertTrue(
                has_permission(user, self.team, "manage_devices"), f"{user.email} should have manage_devices"
            )
        for user in [self.operator, self.viewer]:
            self.assertFalse(
                has_permission(user, self.team, "manage_devices"), f"{user.email} should NOT have manage_devices"
            )
        ...
        for user in [self.admin, self.manager, self.operator, self.viewer]:
            self.assertFalse(
                has_permission(self.owner, self.team, "manage_billing") is False
                if user == self.owner
                else has_permission(user, self.team, "manage_billing"),
                f"{user.email} should NOT have manage_billing",
            )
