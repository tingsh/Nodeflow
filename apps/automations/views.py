...
from .forms import AutomationForm, ConditionFormSet, ActionFormSet
...
class AutomationUpdateView(PermissionRequiredMixin, UpdateView):
    ...
    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        if self.request.POST:
            context['conditions'] = ConditionFormSet(self.request.POST, instance=self.object, form_kwargs={'team': self.request.team})
            context['actions'] = ActionFormSet(self.request.POST, instance=self.object, form_kwargs={'team': self.request.team})
        else:
