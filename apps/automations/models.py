...
class Automation(BaseTeamModel):
    ...
    trigger_logic = models.CharField(max_length=10, choices=TriggerLogicChoices.choices, default=TriggerLogicChoices.AND)
    cooldown_minutes = models.PositiveIntegerField(default=5, help_text=_("Minutes to wait after execution before this automation can fire again"))
...
class AutomationCondition(BaseTeamModel):
    ...
    threshold = models.CharField(max_length=255, blank=True, help_text=_("Can be a number, string, or left blank for boolean checks."))
    
    duration_seconds = models.PositiveIntegerField(default=0, help_text=_("How long the condition must remain true before acting (sustained trigger)"))
...
class AutomationAction(BaseTeamModel):
    ...
    target_device = models.ForeignKey(Device, on_delete=models.CASCADE, null=True, blank=True, related_name='automation_actions')
