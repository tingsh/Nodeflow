...
def get_z_score_insight(device, key):
    ...
    stats = TelemetryData.objects.filter(
        device=device, key=key, timestamp__gte=lookback, value_numeric__isnull=False
    ).aggregate(avg=Avg("value_numeric"), std=StdDev("value_numeric"))
...
def get_weekly_trend_insight(device, key):
    ...
    current_avg = TelemetryData.objects.filter(device=device, key=key, timestamp__gte=hour_ago).aggregate(
    ...
    last_week_avg = TelemetryData.objects.filter(
        device=device, key=key, timestamp__gte=last_week_start, timestamp__lte=last_week_end
