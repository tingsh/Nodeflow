...
class ChatAIFeatureTest(TestCase):
    def setUp(self):
        ...
        self.device = Device.objects.create(
            team=self.team, site=self.site, name="Power Meter 1", device_type="power_meter", energy_category="utility"
        )

        # Add some telemetry
        now = timezone.now()
        TelemetryData.objects.create(
            device=self.device, timestamp=now - timezone.timedelta(minutes=5), key="active_power", value_numeric=150.5
        )
        TelemetryData.objects.create(
            device=self.device, timestamp=now - timezone.timedelta(minutes=10), key="active_power", value_numeric=140.0
        )
    ...
    def test_tool_executor_energy_aggregation(self):
        ...
        data = executor.get_energy_data(
            device_ids=[self.device.id], keys=["active_power"], start_date=start, end_date=end, aggregation="hour"
        )
