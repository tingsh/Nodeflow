...
class NodeflowToolExecutor:
        ...
        for device in devices:
            ...
            if latest:
                ...
                readings = TelemetryData.objects.filter(
                    device=device, timestamp__gte=latest.timestamp - timedelta(seconds=10)
                )
                for r in readings:
    ...
    def get_energy_data(self, device_ids, keys, start_date, end_date, aggregation="hour"):
        ...
        qs = TelemetryData.objects.filter(
            device__in=devices, key__in=keys, timestamp__range=(start, end), value_numeric__isnull=False
        )

        from django.db.models.functions import TruncDay, TruncHour, TruncWeek

        trunc_func = {"hour": TruncHour, "day": TruncDay, "week": TruncWeek}.get(aggregation, TruncHour)

        data = (
            qs.annotate(bucket=trunc_func("timestamp"))
            .values("bucket", "device__name", "key")
            .annotate(avg_value=Avg("value_numeric"), max_value=Max("value_numeric"), min_value=Min("value_numeric"))
            .order_by("bucket")
        )
