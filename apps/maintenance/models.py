...
class TicketTemplate(BaseTeamModel):
    ...
    checklist = models.JSONField(default=list, help_text=_("List of checklist items like {'task': 'Check oil', 'required': True}"))
...
class MaintenanceTicket(BaseTeamModel):
    ...
    reported_by = models.ForeignKey(CustomUser, on_delete=models.SET_NULL, null=True, blank=True, related_name='reported_tickets')
    
    # Traceability
    alert_reference = models.CharField(max_length=100, blank=True, help_text=_("UUID of the alert that triggered this ticket, if any"))
    ...
    checklist_state = models.JSONField(default=list, help_text=_("Instance copy of the template checklist with completion states"))
