...
- 2026-07-28: Fixed the local `devices.0027_managed_gateway_hardening` migration backfill so historical `Gateway` rows use `created_at` when no `claimed_at` field exists on the migration-state model. Local migration now applies cleanly, `devices.0027` shows applied, and Django check, migration drift check and Ruff on the migration file pass with only existing warnings.
