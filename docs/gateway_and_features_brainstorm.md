...
To roleplay pulling data from this PLC into Nodeflow, here is what we'll do:
...
3. **The Gateway Script:** We will build a simple Python script running on your laptop. It will use the Python `opcua` or `pymodbus` library to read `MW100` over your local Ethernet cable, format it into JSON, and push it to Nodeflow (Mosquitto) on `localhost`.

---

## 3. The ThingsBoard Fork: Software & Repositories

**Your Question:** *"For the forked python TB iot gateway, do we do this in a separate code repo? Or do we do this in this current code repo Nodeflow? What are their server dependencies?"*

### One Repo or Two?
**Two completely separate repositories.**
* `Repo 1 (Nodeflow Cloud)`: What we have now (Django, HTMX, Tailwind, TimescaleDB). It runs in the cloud (or your localhost docker).
* `Repo 2 (Nodeflow Edge)`: The Python script that sits in the factory. It reads Modbus/OPC-UA and publishes MQTT.
...
**Recommendation for Now:** Because TB Gateway is large, for our *very first Siemens test*, I recommend we don't even use TB Gateway. We write a custom 50-line Python script using `pymodbus` or `asyncua` just to prove we can read the S7-1200 and get it into Nodeflow. Once that works, we can talk about building the full Edge Gateway software.

---

## 4. Brainstorming Missing Features for an "IIoT Shopify"

If Nodeflow is to dominate, here are high-value features missing from the platform that factories actively pay for:

| Feature | Concept | Why Customers Buy It |
|---------|---------|----------------------|
| **Write-back / Control Commands** | Not just reading data, but turning things ON/OFF. From the web dashboard, click a toggle switch that sends a Modbus write command back to the PLC to stop a motor. | Transforms Nodeflow from a "dashboard" into a "Remote SCADA" system. |
| **Logic & Automations** | "If Temperature > 50°C for 5 minutes, automatically turn on Chiller 2 via Modbus." Nodeflow handles logic in the cloud. | Saves labor costs. Automation without needing a PLC programmer. |
...
2. **Build a Python PLC Script (Repo 2)**. A lightweight script on your laptop to read data from your S7-1200 via Ethernet and push it to Nodeflow's local MQTT broker. 
